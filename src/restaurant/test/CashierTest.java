...
public class CashierTest extends TestCase
{
	...
	public void testOneNormalCustomerScenario()
	{
		...
		//check preconditions
		assertEquals("Cashier should have 0 bills in it. It doesn't.",cashier.uncomputedChecks.size(), 0);		
		assertEquals("CashierAgent should have an empty event log before the Cashier's HereIsBill is called. Instead, the Cashier's event log reads: "
						+ cashier.log.toString(), 0, cashier.log.size());
		
		//step 1 of the test
		
		//String choice, int tableNum, CustomerAgent c, WaiterAgent w
		//Check check = new Check("steak", 1, customer, waiter);
		...
		assertTrue("Cashier's scheduler should have returned true (needs to react to 2's ReadyToPay), but didn't.", 
